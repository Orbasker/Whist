...
class EmailService:
    ...
    def _validate_frontend_url(self):
        ...
        if settings.is_production:
            ...
            if ".api." in url or url.endswith(".api"):
                issues.append(
                    f"FRONTEND_URL appears to point to API domain ({self.frontend_url}), should point to frontend domain"
                )
    ...
    async def send_invitation(
        ...
        logger.info(
            f"Generating invitation link: {invitation_link} (frontend_url={self.frontend_url})"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to send invitation email to {email}: {e.response.status_code} - {e.response.text}"
            )
            return False
