...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, game_id: str):
        ...
        logger.info(
            f"WebSocket connected to game {game_id}. Total connections: {len(self.active_connections[game_id])}"
        )
            ...
            for player_idx, bid in state["bid_selections"].items():
                await websocket.send_text(
                    json.dumps(
                        {
                            "type": "bid_selection",
                            "data": {"player_index": int(player_idx), "bid": bid},
                        }
                    )
                )
                    ...
                    json.dumps(
                        {
                            ...
                            "data": {"player_index": int(player_idx), "trick": trick},
                        }
                    )
                )
                    ...
                    json.dumps(
                        {
                            ...
                            "data": {"trump_suit": state["trump_selection"]},
                        }
                    )
                )
            await websocket.send_text(json.dumps({"type": "phase_update", "phase": state["phase"]}))
