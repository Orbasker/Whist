...
async def create_invitations(
    ...
    if player_indices:
        if len(player_indices) != len(emails):
            raise HTTPException(
                status_code=400, detail="player_indices length must match emails length"
            )
        if not all(0 <= idx <= 3 for idx in player_indices):
            raise HTTPException(status_code=400, detail="player_indices must be between 0 and 3")
    else:
    ...
    for idx in player_indices:
        if idx < len(player_user_ids) and player_user_ids[idx] is not None:
            raise HTTPException(status_code=400, detail=f"Player seat {idx} is already taken")
...
async def accept_invitation(
    ...
    logger.info(
        f"Invitation acceptance attempt: game_id={game_id}, invitee_email={invitee_email}, player_index={player_index}"
    )
    ...
    if player_index < len(player_user_ids):
        ...
        if existing_id is not None:
            ...
            if existing_id_str == user_id_str:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Player seat {player_index} is already taken"
                )
