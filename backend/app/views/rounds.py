...
async def submit_bids(
    ...
    try:
        ...
        return {"game": game, "round_mode": round_mode, "total_bids": total_bids}
...
async def submit_tricks(
    ...
    try:
        ...
        await connection_manager.broadcast_game_update(
            str(game_id), updated_game.model_dump(mode="json")
        )
        await connection_manager.broadcast_phase_update(str(game_id), "bidding")

        return {"game": updated_game, "round": RoundResponse.model_validate(round_obj)}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
