...
async def websocket_endpoint(
    ...
    try:
        ...
        if game:
            ...
            await websocket.send_text(
                json.dumps({"type": "game_update", "game": game.model_dump(mode="json")})
            )

            await websocket.send_text(json.dumps({"type": "phase_update", "phase": phase}))
                ...
                if message_type == "submit_bids":
                    ...
                    try:
                        ...
                        if game:
                            ...
                            await connection_manager.broadcast_game_update(
                                game_id, game.model_dump(mode="json")
                            )
                            ...
                            broadcast_message = json.dumps(
                                {
                                    ...
                                    "data": {"bids": bids, "trump_suit": trump_suit},
                                }
                            )
                    ...
                    try:
                        ...
                        if result:
                            ...
                            await connection_manager.broadcast_message(
                                game_id, tricks_submitted_message
                            )
                                ...
                                json.dumps(
                                    {
                                        ...
                                        "data": {"tricks": tricks, "round": round_response},
                                    }
                                ),
                            )
                    except ValueError as e:
                    ...
                    logger.info(
                        f"Received bid_selection: player {player_index}, bid {bid} for game {game_id}"
                    )

                    if player_index is not None and bid is not None:
                        await connection_manager.broadcast_bid_selection(game_id, player_index, bid)
                    ...
                    logger.info(
                        f"Received trick_selection: player {player_index}, trick {trick} for game {game_id}"
                    )
                    ...
                    if trump_suit is not None:
                        await connection_manager.broadcast_trump_selection(game_id, trump_suit)
                    ...
                    if player_index is not None:
                        await connection_manager.broadcast_message(
                            game_id,
                            json.dumps(
                                {"type": "bet_locked", "data": {"player_index": player_index}}
                            ),
                        )
