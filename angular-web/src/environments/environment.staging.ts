export const environment = {
  production: false,
  // Staging API URL - replace with your staging backend URL
  apiUrl: 'https://api-staging.yourdomain.com/api/v1',
  // Staging Neon Auth URL - get this from your Neon project dashboard
  // Go to: Neon Dashboard → Users → Configuration → Auth URL
  // Format: https://ep-xxx.neonauth.region.aws.neon.tech/neondb/auth
  authUrl: 'https://ep-xxx-staging.neonauth.region.aws.neon.tech/neondb/auth',
  useSupabaseRealtime: true,
  supabaseUrl: '',
  supabaseAnonKey: '',
};
