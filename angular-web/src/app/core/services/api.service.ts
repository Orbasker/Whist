...
export class ApiService {
  ...
  listGames(): Observable<GameState[]> {
    return this.http
      .get<GameState[]>(`${this.baseUrl}/games`)
      .pipe(retry(2), catchError(this.handleError));
  }

  createGame(players: string[], name?: string): Observable<GameState> {
    return this.http
      .post<GameState>(`${this.baseUrl}/games`, { players, name })
      .pipe(retry(2), catchError(this.handleError));
  }

  getGame(gameId: string): Observable<GameState> {
    return this.http
      .get<GameState>(`${this.baseUrl}/games/${gameId}`)
      .pipe(retry(2), catchError(this.handleError));
  }

  updateGame(gameId: string, update: Partial<GameState>): Observable<GameState> {
    return this.http
      .put<GameState>(`${this.baseUrl}/games/${gameId}`, update)
      .pipe(retry(2), catchError(this.handleError));
  }

  deleteGame(gameId: string): Observable<void> {
    return this.http
      .delete<void>(`${this.baseUrl}/games/${gameId}`)
      .pipe(retry(2), catchError(this.handleError));
  }
  ...
  getRounds(gameId: string): Observable<Round[]> {
    return this.http
      .get<Round[]>(`${this.baseUrl}/games/${gameId}/rounds`)
      .pipe(retry(2), catchError(this.handleError));
  }
